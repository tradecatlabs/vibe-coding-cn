...
**这是一个不断生长和自我校正的项目。** 本仓库的统一定位是：中文 Vibe Coding 从入门到精通教程，帮助零基础学习者和开发者从环境配置、工具选择、需求表达、计划拆解、AI CLI 协作、代码验证、Git 迭代一路走到可独立交付项目。**Vibe Coding** 不是“让 AI 随便写”，而是用自然语言定义目标，用 Codex CLI 等 AI 工具执行工程动作，再用文档、测试和版本控制把结果固定下来。本指南强调**规划驱动**、**模块化**、**索引构建**与**强制门禁**，目标是让新手能从 0 开始，也能逐步形成可复用的专业工作流。
