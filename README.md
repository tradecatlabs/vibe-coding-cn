...
**如果你不知道先看哪一块**：按下面关系理解本仓库，先跑通最小闭环，再按需深入。

| 入口 | 作用 | 什么时候用 |
|:---|:---|:---|
| 快速开始 | 零依赖启动器，帮你先装好 Codex CLI 和本地 Agent | 还没有环境，或不知道第一步怎么开始 |
| 从零开始 | 系统学习路线，从网络、CLI、开发环境到 Git 闭环 | 已能按步骤学习，想补齐基础能力 |
| workflow | 做项目时的默认推进顺序、质量门禁和交付闭环 | 已有任务，想知道下一步该让 Agent 做什么 |
| prompts / skills | 可复用的提示词和技能入口 | 想把一次成功经验沉淀成可重复流程 |
| 演示与产出 | 看最终会产出什么，以及各目录如何协作 | 想确认 demo、示例和仓库资产该怎么使用 |
