...
**一套面向零基础学习者的 Vibe Coding 入门到精通教程**
...
**这是一个不断生长和自我校正的项目。** 本仓库的统一定位是：帮助零基础学习者从环境配置、工具选择、需求表达、计划拆解、AI CLI 协作、代码验证、Git 迭代一路走到可独立交付项目。**Vibe Coding** 不是“让 AI 随便写”，而是用自然语言定义目标，用 Codex CLI 等 AI 工具执行工程动作，再用文档、测试和版本控制把结果固定下来。本指南强调**规划驱动**、**模块化**与**索引构建**，目标是让新手能从 0 开始，也能逐步形成可复用的专业工作流。
...
5. [Codex-CLI配置](./assets/documents/guides/getting-started/Codex-CLI配置.md) - 默认 AI CLI 路线，使用 gpt-5.5 与仓库配置基线
6. [OpenCode-CLI配置](./assets/documents/guides/getting-started/OpenCode-CLI配置.md) - 备选 AI CLI 路线，适合暂时无法使用 Codex CLI 的场景
...
*   [**OpenCode CLI**](./assets/documents/guides/getting-started/OpenCode-CLI配置.md): 备选 AI CLI 路线，适合 Codex CLI 不可用或需要免费/本地模型的场景。
