...
# vibe-coding-cn：中文 Vibe Coding 默认工作站
...
`vibe-coding-cn` 是一套中文 Vibe Coding 工作站，帮助用户通过 AI 结对编程，从想法、需求、技术方案、任务拆解到编码、测试、部署和复盘，系统地把想法变成可运行产品。
...
> **不是 Prompt 集合，而是中文 Vibe Coding 默认工作站。**

## 🎲 前言

**这是一个不断生长和自我校正的项目。** 本仓库的统一定位是：中文 Vibe Coding 默认工作站，帮助零基础学习者和开发者从环境配置、工具选择、需求表达、计划拆解、AI CLI 协作、代码验证、Git 迭代一路走到可独立交付项目。**Vibe Coding** 不是“让 AI 随便写”，而是用自然语言定义目标，用 Codex CLI 等 AI 工具执行工程动作，再用文档、测试和版本控制把结果固定下来。本指南强调**规划驱动**、**模块化**、**索引构建**与**强制门禁**，目标是让新手能从 0 开始，也能逐步形成可复用的专业工作流。
