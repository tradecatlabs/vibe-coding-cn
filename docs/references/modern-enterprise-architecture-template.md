...
**专项修订**：V2.44 在 V2.43 基础上新增基线状态对账报告，把生命周期状态机、版本控制面、发布证据包、支持矩阵、采纳总账、符合性声明和审计导出中的版本状态统一做机器对账，阻断状态字段互相漂移。
...
| 版本 | 状态 | 说明 |
| ---- | ---- | ---- |
| `V2.44` | `Baseline Candidate` | 用作可执行企业标准起点；包含机器可读版本清单、控制项覆盖清单、77 组 starter kit schema/example、基线状态对账报告、基线生命周期状态机、基线就绪评分卡、基线例外总账、基线回滚验证记录、基线通知确认总账、基线验证环境锁定、基线制品清单、基线符合性声明、基线发布列车、基线支持矩阵、基线采纳总账、基线兼容性总账、基线发布证据包、版本控制面、外部标准版本锁定、企业执行控制面、合规等级、门禁决策、证据新鲜度、例外放行、break-glass、季度复核、仓库变更控制、远端保护漂移整改、控制证据映射、审计导出清单、审计导出自动化、控制评估报告、架构基线变更记录、架构决策记录、AI 证据账本、微调运行证据、AI 事件响应 playbook、OSCAL 交换映射、POA&M 整改计划、企业架构风险登记、审计导出门禁、审计导出完整性清单、审计导出 provenance statement、审计导出签名策略、审计导出签名验签回执、严格 schema 模式、访问复核、密钥轮换、漏洞修复、事故复盘、可靠性、数据治理、AI 运行、GitOps 安全、供应链证据链一致性和自动化校验入口 |
...
9. Starter Kit：提供 `内部 starter kit` 下的 77 组 schema/example、嵌套字段校验、格式校验、可靠性、数据治理、AI 运行、基线状态对账报告、基线生命周期状态机、基线就绪评分卡、基线例外总账、基线回滚验证记录、基线通知确认总账、基线验证环境锁定、基线制品清单、基线符合性声明、基线发布列车、基线支持矩阵、基线采纳总账、基线兼容性总账、基线发布证据包、版本控制面、外部标准版本锁定、企业执行控制面、合规等级、门禁决策、仓库变更控制、远端保护漂移整改、AI 证据账本、微调运行证据、AI 事件响应 playbook、GitOps 安全、架构决策记录、风险登记、证据链验真字段和示例跨文件一致性检查。
...
V2.44 starter kit 还提供以下可执行契约模板：
...
versionGovernance:
  ...
  rollback:
    previousBaseline: V2.43
    rollbackCommit: <git-sha-of-previous-baseline>
    rollbackTag: architecture/v2.43-candidate
...
1. 任意基线都能从 `version-governance.yaml` 追溯到源 commit、release tag、版本清单、基线生命周期状态机、基线状态对账报告、基线就绪评分卡、基线例外总账、通知确认总账、回滚验证记录、验证环境锁、基线制品清单和审计导出。
...
baselineReleaseEvidence:
  evidenceId: bre-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
  promotion:
...
2. `immutableRefs.sourceCommit`、`releaseTag`、生命周期状态机摘要、状态对账报告摘要、验证环境锁摘要、基线制品清单摘要、基线就绪评分卡摘要、基线例外总账摘要、审计导出摘要和控制覆盖摘要必须来自同一基线。
...
2. 任意证据包都能证明版本清单、生命周期状态机、状态对账报告、验证环境锁、基线制品清单、基线就绪评分卡、基线例外总账、控制覆盖、标准基线和审计导出摘要一致。
...
baselineCompatibilityLedger:
  ledgerId: bcl-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
  change:
...
baselineAdoptionLedger:
  ledgerId: bal-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
  scope:
  ...
  targets:
    - target: domain:order
      ...
      evidence:
        ...
        - governance/control-plane/version-governance.yaml
        - governance/evidence/control-assessments/order-domain-v244.yaml
    - target: ai-product:customer-service-agent
      owner: team-support-ai
      currentBaseline: V2.44
      targetBaseline: V2.44
    ...
    - target: platform-golden-path:microservice
      ...
      evidence:
        ...
        - governance/evidence/platform/microservice-golden-path-v244.yaml
  exceptions:
    active:
...
baselineSupportMatrix:
  matrixId: bsm-20260602-mea-v244
  owner: architecture-governance-board
  currentBaseline: V2.44
...
baselineConformanceClaim:
  claimId: bcc-20260602-order-command-service-v244
  asset:
  ...
  evidence:
    ...
    controls:
      - governance/evidence/control-assessments/order-command-service-v244.yaml
      - governance/control-plane/release-gate-decision.yaml
    supplyChain:
...
baselineArtifactInventory:
  inventoryId: bai-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
  sourceCommit: <git-sha-of-baseline-commit>
...
baselineNotificationLedger:
  ledgerId: bnl-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
  releaseTrain: governance/evidence/release-trains/baseline-release-train.yaml
...
baselineRollbackVerification:
  verificationId: brv-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
  currentBaseline:
  ...
  checks:
    - checkId: git-tag-checkout
      command: git checkout architecture/v2.43-candidate
      expectedResult: pass
      result: pass
...
baselineExceptionLedger:
  ledgerId: bel-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
...
baselineReadinessScorecard:
  scorecardId: brs-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
...
baselineLifecycleStateMachine:
  machineId: blsm-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
...
baselineStateReconciliationReport:
  reportId: bsr-20260602-mea-v244
  baselineId: mea-v2.44-20260602
  documentVersion: V2.44
  owner: architecture-governance-board
...
2. 对账输入至少覆盖生命周期状态机、版本控制面、发布证据包、发布列车、支持矩阵、采纳总账、符合性声明、制品清单、验证锁和审计导出清单。
...
5. `writeBack` 必须证明状态迁移后的版本控制面、支持矩阵、发布证据摘要、采纳总账和审计导出清单已经更新。
...
该命令是仓库内零依赖 starter gate，用于校验版本清单、控制项覆盖清单、77 组示例的 JSON Schema 子集、YAML 示例、嵌套必填字段、格式约束、数值阈值、严格 schema 模式、基线状态对账报告、基线生命周期状态机、基线就绪评分卡、基线例外总账、基线回滚验证记录、基线通知确认总账、基线验证环境锁定、基线制品清单、基线符合性声明、基线发布列车、基线支持矩阵、基线采纳总账、基线兼容性总账、基线发布证据包、版本控制面、外部标准版本锁定、企业执行控制面、合规等级、门禁决策、仓库变更控制、远端保护漂移整改、访问复核、密钥轮换、漏洞修复、事故复盘、证据新鲜度、AI 证据账本、微调运行证据、AI 事件响应 playbook、控制证据映射、审计导出清单、审计导出自动化命令、控制评估报告、架构基线变更记录、架构决策记录、OSCAL 交换映射、POA&M 整改计划、企业架构风险登记、审计导出门禁、审计导出完整性清单、审计导出 provenance statement、审计导出签名策略、审计导出签名验签回执、未知字段阻断、证据链字段和示例间一致性。企业生产落地时应优先接入成熟校验器，例如 JSON Schema draft 2020-12 validator、YAML parser、OpenAPI / AsyncAPI checker、OPA / Cedar / Kyverno policy test、SLSA / Sigstore verifier、OpenTelemetry collector、OpenCost / FOCUS 工具链、IAM / Secret 管理系统、漏洞管理平台、事故管理系统、OSCAL 工具链和 GitOps diff 工具；本仓库脚本只作为 starter kit 的最小可执行证明。
...
| 对标来源 | 关键结论 | 本文档落点 |
| -------- | -------- | ---------- |
| Semantic Versioning / Conventional Commits / Keep a Changelog | 版本号、提交语义和变更记录必须表达兼容性、影响面和升级意图 | 增加 `version-governance.yaml`、`baseline-lifecycle-state-machine.yaml`、`baseline-state-reconciliation-report.yaml`、`baseline-readiness-scorecard.yaml`、`baseline-rollback-verification.yaml`、`baseline-notification-ledger.yaml`、`baseline-verification-lock.yaml`、`baseline-artifact-inventory.yaml`、`baseline-conformance-claim.yaml`、`baseline-release-train.yaml`、`baseline-support-matrix.yaml`、`baseline-compatibility-ledger.yaml`、发布通道、生命周期状态机、状态对账、基线不变量、就绪评分、回滚验证、通知确认、验证锁、制品清单、资产声明、发布节奏、支持窗口、兼容窗口、消费者迁移和回滚入口 |
