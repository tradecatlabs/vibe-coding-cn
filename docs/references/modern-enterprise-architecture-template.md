...
**专项修订**：V2.19 在 V2.18 基础上新增审计导出签名策略，让 provenance payload 的摘要、签名方式、验签命令和外部签名交接进入可执行门禁。
...
| 版本 | 状态 | 说明 |
| ---- | ---- | ---- |
| `V2.19` | `Baseline Candidate` | 用作可执行企业标准起点；包含机器可读版本清单、控制项覆盖清单、50 组 starter kit schema/example、控制证据映射、审计导出清单、审计导出自动化、控制评估报告、架构基线变更记录、OSCAL 交换映射、审计导出门禁、审计导出完整性清单、审计导出 provenance statement、审计导出签名策略、严格 schema 模式、访问复核、密钥轮换、漏洞修复、事故复盘、证据新鲜度、可靠性、数据治理、AI 运行、GitOps 安全、供应链证据链一致性和自动化校验入口 |
...
9. Starter Kit：提供 `docs/references/modern-enterprise-architecture-kit/` 下的 50 组 schema/example、嵌套字段校验、格式校验、可靠性、数据治理、AI 运行、GitOps 安全、证据链验真字段和示例跨文件一致性检查。
...
V2.19 starter kit 还提供以下可执行契约模板：
...
34. `baseline-change-record.yaml`：架构基线变更版本、前序版本、影响分析、审批、验证命令、回滚计划和留存复审。
35. `oscal-export-profile.yaml`：OSCAL catalog、component-definition、SSP、assessment-results 和 POA&M 交换映射。
36. `audit-export-gate.yaml`：审计导出命令、本地质量门禁、输出不变量、OSCAL 摘要一致性和留存复审。
37. `audit-export-integrity.yaml`：审计导出生成物、SHA-256 摘要、源制品哈希和防篡改校验。
38. `audit-export-provenance.yaml`：审计导出 subject、构建定义、导出命令、源码提交和源证据依赖追溯。
39. `audit-export-signing-policy.yaml`：审计导出 provenance payload 摘要、Cosign 签名交接、验签命令和本地门禁边界。
...
| 审计导出自动化 | 校验命令、导出脚本、JSON 包、Markdown 报告、OSCAL 摘要、完整性清单、provenance statement 和签名策略 | `make export-modern-architecture-audit`、导出脚本、build 输出 | 审计包只能手工拼接、未先校验、缺制品哈希、生成来源或签名交接 |
...
该命令是仓库内零依赖 starter gate，用于校验版本清单、控制项覆盖清单、50 组示例的 JSON Schema 子集、YAML 示例、嵌套必填字段、格式约束、数值阈值、严格 schema 模式、访问复核、密钥轮换、漏洞修复、事故复盘、证据新鲜度、控制证据映射、审计导出清单、审计导出自动化命令、控制评估报告、架构基线变更记录、OSCAL 交换映射、审计导出门禁、审计导出完整性清单、审计导出 provenance statement、审计导出签名策略、未知字段阻断、证据链字段和示例间一致性。企业生产落地时应优先接入成熟校验器，例如 JSON Schema draft 2020-12 validator、YAML parser、OpenAPI / AsyncAPI checker、OPA / Cedar / Kyverno policy test、SLSA / Sigstore verifier、OpenTelemetry collector、OpenCost / FOCUS 工具链、IAM / Secret 管理系统、漏洞管理平台、事故管理系统、OSCAL 工具链和 GitOps diff 工具；本仓库脚本只作为 starter kit 的最小可执行证明。
...
24. 审计导出 JSON、Markdown、OSCAL 摘要的版本、pair 数、控制数和评估状态不变量检查。
25. 审计导出生成物 SHA-256 摘要、源制品哈希和完整性清单防篡改检查。
26. 审计导出 provenance subject digest、构建定义、源码提交和源证据依赖检查。
27. 审计导出签名策略、payload 摘要、验签命令、bundle 路径和外部签名交接检查。
...
| 审计导出自动化证据 | 导出命令、JSON 包、Markdown 报告、OSCAL 摘要、完整性清单、provenance statement、签名策略和校验结果 | 治理团队和平台团队 | 覆盖每次正式审计导出 |
...
| OSCAL 交换证据 | OSCAL 模型映射、导出摘要、控制目录、评估结果和 POA&M 状态 | 治理团队和安全团队 | 覆盖每次正式审计导出 |
